...
-- 한번에 데이터를 변경하는 경우 동시에 진행이되기 때문에 원하는 결과값이 나오지 X
...
-- email E, S 포함되어 있는 사워을 구하시오
select * from emplyees;
select email form employees 
where email like '%E%' or email like '%S%;;
select email from emplyees;

select email from employees 
where email like '%E%' and mail like ';

select * from stuscore;

-- 2개 order by 정렬, 1차 우선 정렬하고
