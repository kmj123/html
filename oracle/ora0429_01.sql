...
create table mem1(
memNo number,     -- 최대길이로 표현 가능, 용량을 많이 차지
memNo2 number(3), -- 0~999 까지 가능
memNo3 number(4,2) -- 4자리 소수점 2자리, 소수점 1개의 자리 차지
);

---------------------------------------------
-- number : 숫자데이터, 소수점 표현 가능. int, float
-- number(4,2) : 총 자리수 4자리, 소수점 자리수 2자리 -> 0.0 ~ 99.99
...
-- (4,2) -> 소수점 포함 총 4자리, 소수점 두자리를 무조건 포함해야함
-- (6,2) -> 9999.99
...
update mem1 set memno=33333333 where memno3 = 99.9;
...
-- 25/04/29
-- 날짜 포맷 변경 방법 date타입 -> char 타입ㄱ으로 변경해서 날짜 형태를 바꿀수있음
select to_char(sdate,'yyyy-mm-dd hh:mi:ss') from date1;

-- char 타입: 고정형 문자형 타입, varchar2타입: 가변형 문자형 타입
create table mem2 (
...
-- 14자리 -> 10자리만 입력해도 14자리를 차지
-- char(고정형): varchar2 보다 속도가 빠르다
-- varchar2(비고정형): char보다 느리지만 비정형 수에 대해 공간활용이 좋다.
...
-- nchar 국제언어 고정형 문자열 타입, nvarchar2 국제언어 가변형문자열 타입 
create table mem3 (
gender1 char(1),  
gender2 char(2),
gedenr3 char(3),
gender4 nchar(1)    -- 국제언어 1개 단어를 저장형태 타입 2byte 사용
);
...
-- char(1~2): 영문 1byte, 국문 3byte: not enough values
insert into mem3(gedenr3) values('남');
...
-- null 값에 +,-,*,/ 전부 null 변경
-- nvl(): 값이 null인 경우 commission_pct 값을 0으로 대체
...
-- manger_id타입: number타입, 글자를 입력할수없음
select manager_id,nvl(manager_id,'CEO') from employees;

-- salary, salary*12, salary*commission_pct,salary+(salary*commission_pct) 출력
-- salary*12*1438 천단위 표시를해서 출력
select salary,salary*12,salary*commission_pct,salary+(salary*nvl(commission_pct,0))from employees;

select salary*12*1438 as "달러연봉", to_char(salary*12*1438,'999,999,999')as "한화 연봉" from employees;

select * from member;
desc member;

-- concat 함수: 문자열 컬럼을 합쳐주는 함수: ||, concat
-- select id+pw from member;
select id||','||pw from member;
...
select period,region,gubun,to_char(loan_jan_amt*10000000,'999,999,999,999') as "천만원" from kor_loan_status;

-- 25000.0억 이상 역순정렬로 출력하시오
select period, region, gubun, loan_jan_amt from kor_loan_status where loan_jan_amt >=40000 order by loan_jan_amt desc;

-- salary가 4000인 사원 출력  true =, false !=
select emp_name,salary from employees where salary != 4000;

-- 4000, 6000, 7000
select emp_name,salary from employees where salary = 4000;
select emp_name,salary from employees where salary = 6000;
select emp_name,salary from employees where salary = 7000;

select emp_name,salary from employees where salary = 4000 or salary=6000 or salary=7000;
select emp_name,salary from employees where salary in (4000,6000,7000);
...
-- memno3 -99.99, memno 50000, memno2 1000
select * from mem1;
update mem1 set memno=50000, memno2=10 where memno3=-99.99;

-- memno2 = 999 인 테이블에서 memno=5000000, memno3 = 50.05
update mem1 set memno=5000000, memno3=50.05 where memno2=999;

desc mem2;
select * from mem2;
-- id aaa1111을 찾아서, 주민번호 0101010-2222222, 국어점수 80, 영어 95 으로 변경하시오
update mem2 set juminno='010101-2222222', kor=80, eng=95 where id='aaa1111';
...
-- 컬럼 5개만 복사 sno, name, kor, eng, math 
create table stuscore3 as select sno,name,kor,eng,math from stuscore;

-- stuscore3 번호,국어,영어,수학,합계,평균을 출력해보시오.
-- 등수 출력
select sno,kor,eng,math,kor+eng+math as "합계",round((kor+eng+math)/3,3) as "평균" from stuscore3 ;
...
-- sno 96,23,3,22...
select sno,rank() over(order by total asc) from stuscore3;
